...
def last_fix_time(user_id: int) -> db.UserTimeInModel or db.UserTimeOutModel or None:
    with db.Session() as session:
...
def create_admin_hours_report(month: int, year: int) -> str:
    ...
    with db.Session() as session:
        for model in (session.query(db.WorkDayModel).filter(and_(
            ...
            data[model.user.name][model.day.day] = model.time
    ...
    report.add_table(
        ['', *range(1, 16), 'Всего'],
        [[name] + [data[name][i] if data[name].get(i) else '' for i in range(1, 16)] +
         [sum(data[name][i] for i in range(1, 16) if data[name].get(i))] for name in data.keys()]
    )

    report.add_h2("Вторая половина:")
    report.add_table(
        ['', *range(16, calendar.monthrange(year, month)[1]), 'Всего', 'За месяц'],
        [[name] + [data[name][i] if data[name].get(i) else '' for i in range(16, calendar.monthrange(year, month)[1])] +
         [sum(data[name][i] for i in range(16, calendar.monthrange(year, month)[1]) if data[name].get(i))] +
         [sum(data[name][i] for i in range(1, calendar.monthrange(year, month)[1]) if data[name].get(i))]
...
def remove_user(user_id: int) -> db.UserModel:
    with db.Session() as session:
        if (user := session.query(db.UserModel).filter(db.UserModel.id == user_id)) is not None:
            session.delete(user)
            session.commit()
            return user
